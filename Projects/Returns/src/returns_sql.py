#todo match on EUR payments, select EUR ...
...
def get_sql_strings(exclude_loans=False):
    ...
    d['actual_payments_combined_date'] = """
    with
    actual_payments as (
        select 
            ap.dwh_country_id, 
        ...
        join base.loan_payback lp on 
            (lp.dwh_country_id=ap.dwh_country_id and lp.fk_loan=ap.fk_loan)
        where 
            (lp.state!='payback_complete' or lp.in_arrears_since is not null or 
             ap.iso_date <=lp.last_payment_date) and
             ap.fk_loan not in {}
    ...
    actual_payments as (
        ...
        where 
            ap.iso_date=
            (date_trunc('MONTH', ap.iso_date) + INTERVAL '1 MONTH - 1 day')::date and
            (lp.state!='payback_complete' or lp.in_arrears_since is not null or 
             ap.iso_date <=lp.last_payment_date) and
             ap.fk_loan not in {}
        WINDOW W_pay as ( partition by ap.dwh_country_id, ap.fk_loan order by ap.iso_date)
    ...
    where  
        -- ap.dwh_country_id=1 and 
        ap.iso_date <=current_date and  ap.fk_loan not in {0}

    order by dwh_country_id,fk_loan,iso_date ;
    """.format(excluded_loans)

    d['payment_plans_combined']="""
    SELECT pp.dwh_country_id, pp.fk_loan, pp.fk_loan_request,l.loan_nr as loan_request_nr,
        pp.fk_user_investor, pp.fk_user_borrower, pp.country_name,
        pp.nominal_interest_percentage, pp.promo_interest_percentage, pp.has_promo_flag,
        pp.is_repaid_flag,
...
# load combined payment plan up to system date (very large), removing cancelled/non funded loans and loans created by lendico 'borrowers'
    ...
    where
    -- pp.dwh_country_id=1 and 
    l.state!='canceled'  and l.originated_since is not null
...
# load loans removing cancelled, not yet originated  loans and loans created by lendico 'borrowers'
    ...
    d['loan_fundings']="""select * from base.loan_funding 
    where 
    --dwh_country_id=1 and 
    fk_loan not in {} and (state='funded') -- or close_reason is not null)""".format(excluded_loans)
